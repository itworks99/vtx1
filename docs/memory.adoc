...
- **Power:** 2mW active, 0.2mW standby
...
- **Cache Miss to RAM:** 8 cycles (80ns @ 100MHz)
- **Cache Miss to Flash:** 12 cycles (120ns @ 100MHz)
- **Write-back Latency:** 4 cycles (40ns @ 100MHz)
...
- **Access Time:** 25ns random access, 10ns sequential
...
* Read Latency: 25ns first access, 10ns sequential
* Program Latency: 200μs typical, 700μs worst-case
* Erase Latency: 1.5ms typical, 3ms worst-case
...
* **Deep Sleep:** Power-gated with state save, <1mW power
...
* Default Active Timeout: 10ms (no access → standby)
* Standby to Sleep Timeout: 100ms
